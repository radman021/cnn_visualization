...
if selected_model:
    folder_tree = build_folder_tree(
        f"storage/{Models[selected_model].value}/filters_linear_comb_weight_viz"
    )
else:
    folder_tree = None
...
if not folder_tree:
    if st.button("Visualize filters") and selected_model:
        ...
        base_dir = f"storage/{model_enum.value}/filters_linear_comb_weight_viz"

        fv = FiltersVisualizer(model)

        with st.spinner(
            f"Saving filter weights as linear combination of previous filters for model {model_enum.value}..."
        ):
            fv.save_all_filters_linear_combinations(base_dir=base_dir)
